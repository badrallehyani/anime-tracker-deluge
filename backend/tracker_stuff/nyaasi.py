...
class NyaasiResult:
    def __init__(self, row_type: str, category: str, name: str, URL: str,
                ID: str, links: dict[str, str], size: str, date: dict, seeders: int, leechers: int, downloads: int):
...
class Nyaasi:
    ...
    def getFileInfo(fileURL: str):
        ...
        return {
            ...
            "name": fileTitle,
            "links": {
                "torrent_file": Nyaasi.base_url + torrentFileURL,
                'magnet': magnet
            },
            'size': fileSize,
            'date': date,
    ...
    def search(keyword: str):
        searchURL = Nyaasi.base_url + "?q=" + keyword

        response = get(searchURL)
        return Nyaasi.parseSearchResponse(response)
    
    def searchByUser(keyword: str, user: str) -> list[NyaasiResult]:
        searchURL = Nyaasi.base_url + "/user/" + user + "?q=" + keyword
    ...
    def parseSearchResponse(response: Response):
        ...
        parsed_table_rows: list[NyaasiResult] = [Nyaasi.parseSearchTableRow(row) for row in table.findAll("tr")]
    ...
    def parseSearchTableRow(row: Tag):
        ...
        URL =               Nyaasi.base_url + row_tds[1].findAll("a")[-1].get("href"),
        ID =                row_tds[1].findAll("a")[-1].get("href").split('/')[-1],
        links = {
            "torrent_file": Nyaasi.base_url + row_tds[2].find("a").get("href"),
